            ...
            if (user) {
                ...
                if (window.api) {
                    try {
                        ...
                        // If API check fails, token is invalid - clear it and show login
                        ...
                        return `
                            <div class="card">
                                ...
                                <p style="color: var(--text-secondary); font-size: 0.85rem; margin-top: 5px;">
                                    Collected by: ${agentName} | Method: ${payment.method || 'Cash'}
                                    ${payment.razorpayPaymentId ? ` | Razorpay ID: ${payment.razorpayPaymentId.substring(0, 12)}...` : ''}
                                </p>
                                ${payment.status === 'pending' ? `<p style="color: #ff9800; font-size: 0.85rem; margin-top: 5px;"><strong>⏳ Payment Pending</strong></p>` : ''}
                                ${payment.status === 'failed' ? `<p style="color: #f44336; font-size: 0.85rem; margin-top: 5px;"><strong>❌ Payment Failed</strong></p>` : ''}
                            </div>
                        `;
                    }).join('')
